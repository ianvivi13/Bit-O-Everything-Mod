...
public class PlumTreeGrower extends AbstractTreeGrower {
    ...
    protected Holder<? extends ConfiguredFeature<?, ?>> getConfiguredFeature(RandomSource randomSource, boolean withBees) {
        if (randomSource.nextInt(10) == 0) {
            return withBees ? ModConfiguredFeatures.PLUM_TREE_FANCY_BEES_005.getHolder().get() : ModConfiguredFeatures.PLUM_TREE.getHolder().get();
        } else {
            return withBees ? ModConfiguredFeatures.PLUM_TREE_BEES_005.getHolder().get() : ModConfiguredFeatures.PLUM_TREE_FANCY.getHolder().get();
        }
    }
