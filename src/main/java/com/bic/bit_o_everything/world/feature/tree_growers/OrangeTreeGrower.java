...
public class OrangeTreeGrower extends AbstractTreeGrower {
    ...
    protected Holder<? extends ConfiguredFeature<?, ?>> getConfiguredFeature(RandomSource randomSource, boolean withBees) {
        if (randomSource.nextInt(10) == 0) {
            return withBees ? ModConfiguredFeatures.ORANGE_TREE_FANCY_BEES_005.getHolder().get() : ModConfiguredFeatures.ORANGE_TREE.getHolder().get();
        } else {
            return withBees ? ModConfiguredFeatures.ORANGE_TREE_BEES_005.getHolder().get() : ModConfiguredFeatures.ORANGE_TREE_FANCY.getHolder().get();
        }
    }
